...
// Calculates all the unique averages for an n cell configuration each with values 0 to (k - 1)
// k dictates the possible number range, which effectively is the base. i.e k = 2 is binary. k = 8 is octal
Error CalculateUniqueAverages(float* averages, int length, int k, int totalCells) {
    averages[0] = 0.0f;
    int averageIndex = 1;
    int* cells = malloc(totalCells * sizeof(int));
    if (cells == NULL)
    {
        return GenerateError("Failed to allocate memory; CalculateUniqueAverages, rules.c\n");
    }
    memset(cells, 0, totalCells * sizeof(int));
    cells[0] = 1; // This is done because we skip the 0'th position, whose average is 0;

    for (int i = 1; averageIndex < length; i++)
    {
        float average = (float) Sum(cells, totalCells) / (float) totalCells; 
        ...
        IncrementNumber(cells, totalCells, k);
    }
...
Error CalculateTotalisticRule(TotalisticRule* tr, int k, int code, int neighborhood) {

    unsigned char* pointer = malloc( (neighborhood * k - (neighborhood - 1)) * (sizeof(float) + sizeof(int)) );
    if (pointer == NULL)
    {
        return GenerateError("Failed to allocate memory, CalculateTotalisticRule, rules.c\n");
    }

    tr->neighborhood = neighborhood;
    tr->k = k;
    tr->uniqueAverages = neighborhood * k - (neighborhood - 1); // The theory for this formula and where it comes from is in the theory directory.

    tr->averages = (float*) pointer;
    tr->rule = (int*) (tr->averages + tr->uniqueAverages);
    
    Error error = CalculateUniqueAverages(tr->averages, tr->uniqueAverages, tr->k, tr->neighborhood);
...
int ApplyTotalisticRule(TotalisticRule* tr, int* cells) {

    int sum = Sum(cells, tr->neighborhood);
    float average = (float) sum / (float) tr->neighborhood;
